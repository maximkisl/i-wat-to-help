...
//	FirstVC *viewController = [[FirstVC alloc] initWithNibName:nil bundle:nil];
...
// Вызывается послепользователь изменяет выбор .
- (void)tableView:(UITableView *)aTableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == PAWSettingsTableViewSectionDistance) {
        [aTableView deselectRowAtIndexPath:indexPath animated:YES];

        // if we were already selected, bail and save some work.
		// Если мы уже выбран , залог и сохранить какую-то работу .
...
// Вызывается принажатии кнопки. Посмотреть будут автоматически уволены после этого вызова возвращается
...
// Nil реализация чтобы избежать метод по умолчанию UIAlertViewDelegate , что говорит:
// " Вызывается, когда мы отменяем вид (например .Пользователь нажимает кнопку Home) . Это не вызывается, когдапользователь нажимаеткнопку отмены .
// Если не определена в делегата, мы имитировать щелчок в кнопку отмены "
// Так как у нас " Выйти " в отмене индекса (чтобы получить его изнормальной " Ok то, что Появится диалоговое окно сбился мое лицо "
// Позиция , мы должны иметь дело с последствиями этого.
