...
- (void)fetchConfigIfNeeded {
	...
	if (self.config == nil ||
		...
		// Установить конфигурации в кэше версию и начать извлечение новый конфиг
		self.config = [PFConfig currentConfig];

		// Set the date to current and use it as a flag that config fetch is in progress
		// Установить дату тока и использовать его в качестве флага , что конфигурация выборки в ходе
		...
		[PFConfig getConfigInBackgroundWithBlock:^(PFConfig *config, NSError *error) {
			if (error == nil) {
				...
				// Удалить флаг , чтобы указать , что мы должны refetch конфиг раз
				self.configLastFetchedDate = nil;
			}
		}];
	}
...
- (NSArray *)filterDistanceOptions {
	...
	if (!distanceOptions) {
		...
		// Значение Нет конфигурации , не вернется к значениям по умолчанию
